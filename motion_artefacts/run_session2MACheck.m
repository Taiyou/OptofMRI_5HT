...
%% 1. datapath to the 6 motions data
trans_blue_path = fullfile(pwd, '/sesson2/transgenic/blue');
trans_yell_path = fullfile(pwd, '/sesson2/transgenic/yellow');
wildt_blue_path = fullfile(pwd, '/sesson2/WT/blue');
wildt_yell_path = fullfile(pwd, '//sesson2/WT/yellow');
savepath        = fullfile(pwd, '/figures/motion_artefacts/sesson2');
mkdir(savepath)
...
%end
[sig, p_TW_BY]      = ttest2(SP_trans_blue - SP_trans_yell, SP_wildt_blue - SP_wildt_yell);
[sig, p_stim_TW_BY] = ttest2(SP_trans_blue_stim - SP_trans_yell_stim, SP_wildt_blue_stim - SP_wildt_yell_stim);
...
[sig, p_TW_BY_std]      = ttest2(StdP_trans_blue - StdP_trans_yell, StdP_wildt_blue - StdP_wildt_yell);
[sig, p_stim_TW_BY_std] = ttest2(StdP_trans_blue_stim - StdP_trans_yell_stim, StdP_wildt_blue_stim - StdP_wildt_yell_stim);
...
for ii = ndata+1:ndata+nSubject1,
    ...
    % motion data
    motiondata_xyz(ii,1)     = 20*(SP_trans_yell(ii-ndata, 1) - SP_trans_blue(ii-ndata, 1));
    motiondata_xyz_std(ii,1) = (StdP_trans_yell(ii-ndata, 1) - StdP_trans_blue(ii-ndata, 1));
    % motion data rotation
    motiondata_rot(ii,1)     = SP_trans_yell(ii-ndata, 4) - SP_trans_blue(ii-ndata, 4);
    motiondata_rot_std(ii,1) = StdP_trans_yell(ii-ndata, 4) - StdP_trans_blue(ii-ndata, 4);
    % group label
...
for ii = ndata+1:ndata+nSubject2,
    ...
    motionlabel_rot{ii, :}   = 'Rx';
    % motion data
    motiondata_xyz(ii,1)     = 20*(StdP_wildt_yell(ii-ndata, 1) - SP_wildt_blue(ii-ndata, 1));
    motiondata_xyz_std(ii,1) = (StdP_wildt_yell(ii-ndata, 1) - StdP_wildt_blue(ii-ndata, 1));
    % motion data rotation
    motiondata_rot(ii,1)     = SP_wildt_yell(ii-ndata, 4) - SP_wildt_blue(ii-ndata, 4);
    motiondata_rot_std(ii,1) = StdP_wildt_yell(ii-ndata, 4) - StdP_wildt_blue(ii-ndata, 4);
    % group label
...
for ii = ndata+1:ndata+nSubject1,
    ...
    % motion data
    motiondata_xyz(ii,1)     = 20*(SP_trans_yell(ii-ndata, 2) - SP_trans_blue(ii-ndata, 2));
    motiondata_xyz_std(ii,1) = (StdP_trans_yell(ii-ndata, 2) - StdP_trans_blue(ii-ndata, 2));
    % motion data rotation
    motiondata_rot(ii,1)     = SP_trans_yell(ii-ndata, 5) - SP_trans_blue(ii-ndata, 5);
    motiondata_rot_std(ii,1) = StdP_trans_yell(ii-ndata, 5) - StdP_trans_blue(ii-ndata, 5);
    % group label
    stimtype{ii, :}          = 'trasgenic (n=7)';
end
...
for ii = ndata+1:ndata+nSubject2,
    ...
    motionlabel_rot{ii, :}   = 'Ry';
    % motion data
    motiondata_xyz(ii,1)     = 20*(SP_wildt_yell(ii-ndata, 2) - SP_wildt_blue(ii-ndata, 2));
    motiondata_xyz_std(ii,1) = (StdP_wildt_yell(ii-ndata, 2) - StdP_wildt_blue(ii-ndata, 2));
    % motion data rotation
    motiondata_rot(ii,1)     = SP_wildt_yell(ii-ndata, 5) - SP_wildt_blue(ii-ndata, 5);
    motiondata_rot_std(ii,1) = StdP_wildt_yell(ii-ndata, 5) - StdP_wildt_blue(ii-ndata, 5);
    % group label
    stimtype{ii, :}          = 'wild-type (n=6)';
end
...
for ii = ndata+1:ndata+nSubject1,
    ...
    % motion data
    motiondata_xyz(ii,1)     = 30*(SP_trans_yell(ii-ndata, 3) - SP_trans_blue(ii-ndata, 3));
    motiondata_xyz_std(ii,1) = (StdP_trans_yell(ii-ndata, 3) - StdP_trans_blue(ii-ndata, 3));
    % motion data rotation
    motiondata_rot(ii,1)     = SP_trans_yell(ii-ndata, 6) - SP_trans_blue(ii-ndata, 6);
    motiondata_rot_std(ii,1) = StdP_trans_yell(ii-ndata, 6) - StdP_trans_blue(ii-ndata, 6);
    % group label
    stimtype{ii, :}          = 'trasgenic (n=7)';
end
...
for ii = ndata+1:ndata+nSubject2,
    ...
    motionlabel_rot{ii, :}   = 'Rz';
    % motion data
    motiondata_xyz(ii,1)     = 30*(SP_wildt_yell(ii-ndata, 3) - SP_wildt_blue(ii-ndata, 3));
    motiondata_xyz_std(ii,1) = (StdP_wildt_yell(ii-ndata, 3)- StdP_wildt_blue(ii-ndata, 3));
    % motion data rotation
    motiondata_rot(ii,1)     = SP_wildt_yell(ii-ndata, 6) - SP_wildt_blue(ii-ndata, 6);
    motiondata_rot_std(ii,1) = StdP_wildt_yell(ii-ndata, 6) - StdP_wildt_blue(ii-ndata, 6);
    % group label
    stimtype{ii, :}          = 'wild-type (n=6)';
end   
...
g.draw();
g.export('file_name', 'motion_artefact_xyz sesson2', 'export_path', savepath, 'file_type', 'png')
...
g.results.ind_color(2) = 1
g.draw();
g.export('file_name', 'motion_artefact_rot sesson2', 'export_path', savepath, 'file_type', 'png')
...
g.results.ind_color(2) = 1
g.draw();
g.export('file_name', 'motion_artefact_xyz_stdsesson2', 'export_path', savepath, 'file_type', 'png')
...
g.results.ind_color(2) = 1
g.draw();
g.export('file_name', 'motion_artefact_rot_stdsesson2', 'export_path', savepath, 'file_type', 'png')