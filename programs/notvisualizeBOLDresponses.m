function [AUC1, AUC2, meanTIMESERIES1, meanTIMESERIES2, maxTIMES1, maxTIMES2, Pvalues] = notvisualizeBOLDresponses(BOLDresponse1, BOLDresponse2, ROIname, resultpath, colors, type, stim)
...
for iroi = 1:nROI,
    ...
    temptimemean1              = mean(tempsignal1(:, 21:duration),2);
    temptimemean2              = mean(tempsignal2(:, 21:duration),2);
    
    % mean %BOLD signals, extraction
    meanTIMESERIES1(:, iroi)   = mean(tempsignal1(:, 21:duration),2);
    meanTIMESERIES2(:, iroi)   = mean(tempsignal2(:, 21:duration),2);
    
    [max1, idmaxtimes1]      = max(abs(tempsignal1(:, 21:duration)), [], 2);
    [max2, idmaxtimes2]      = max(abs(tempsignal2(:, 21:duration)), [], 2);
    
    % AUC, extraction
    for isub1 = 1:nSub1,
        temp1                 = cumtrapz(tempsignal1(isub1, 21:duration));
    ...
    for isub2 = 1:nSub2,
        temp2                 = cumtrapz(tempsignal2(isub2, 21:duration));
