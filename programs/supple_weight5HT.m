function [] = supple_weight5HT(htR, err)
% This program is intended to visualize regression weights.
% Figure 5d
% 
...
for k = 1:size(htR,2)
    if k == 1,
        ...
        errorbar(xpos, htR(:,k), err(:,k), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1);
    elseif k==2,
        b(k).CData = [0, 0.2, 0.2];
        xpos = b(k).XData + b(k).XOffset;
        errorbar(xpos, htR(:,k), err(:,k), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1);
